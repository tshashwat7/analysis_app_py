...
HORIZON_CONFIG: Dict[str, Dict] = {
    "intraday":   {"period":"5d",  "interval":"5m",  "warmup_bars":100, "forward_window":78,
                   "maFast":9,   "ema_slow":21,  "atr_period":7},
    "short_term": {"period":"2y",  "interval":"1d",  "warmup_bars":60,  "forward_window":20,
                   "maFast":20,  "ema_slow":50,  "atr_period":14},
    "long_term":  {"period":"5y",  "interval":"1wk", "warmup_bars":40,  "forward_window":12,
                   "maFast":50,  "ema_slow":200, "atr_period":21},
}

SIGNAL_STATES   = {"BUY","SELL","WATCH","BLOCKED","AVOID","HOLD"}
OUTCOME_T1      = "T1_HIT"
OUTCOME_T2      = "T2_HIT"
OUTCOME_SL      = "SL_HIT"
...
def fetch_and_cache_fundamentals(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        fund = {
            "peRatio":              info.get("trailingPE") or 20.0,
            "pbRatio":             info.get("priceToBook") or 2.5,
            "roe":          (info.get("returnOnEquity") or 0.15) * 100,
            "roce":                    (info.get("roce") or info.get("returnOnAssets") or 0.10) * 150,
            "deRatio":            (info.get("deRatio") or info.get("debtToEquity") or 50.0) / 100,
        ...
        with open(path, "w", encoding="utf-8") as f: json.dump(fund, f, indent=2)
        return fund
    except Exception as e:
        log.warning(f"Fundamentals failed {symbol}: {e}")
        return {"peRatio":20,"pbRatio":2.5,"roe":15,
                "roce":14,"deRatio":0.5,
                "promoterHolding":55,"piotroskiF":6,"epsGrowth5y":12,
                "marketCap":5_000_000_000,"revenueGrowth":10,
...
def compute_indicators_from_ohlcv(df: Any, horizon: str) -> Dict[str, Any]:
    ...
    # EMAs
    ef = close.ewm(span=min(cfg["maFast"],n-1), adjust=False).mean()
    ...
    return {
        ...
        "avgVolume30Days": _w(vm_v, 5.0),
        "volume_signature":  {"value":vs,"raw":vs},
        # Price — _extract_price_data reads "price" and "prevClose"
        "price":             _w(px,           5.0),
        "prevClose":        _w(_L(close.shift(1)), 5.0),
        "close":             _w(px,           5.0),
...
class BarSignal:
    ...
    rrRatio:Optional[float]; sl_atr_mult:Optional[float]
...
class PipelineAdapter:
    ...
    def _run_isolated(self,symbol,horizon,indicators,fundamentals,
        ...
        ef_v    = _nv(indicators,"maFast",price)
        ...
        de=fundamentals.get("deRatio",fundamentals.get("debtToEquity",0.8))
        ...
        return BarSignal(
            ...
            rrRatio=_f(rr),sl_atr_mult=_f(sl_atr),
    ...
    def _from_plan(self,symbol,horizon,bar_idx,bar_date,price,atr,
        ...
        return BarSignal(
            ...
            rrRatio=rr,sl_atr_mult=_f(sl_atr),
    ...
    def _from_ctx(self,symbol,horizon,bar_idx,bar_date,price,atr,
        ...
        conf_floor = c_data.get("floor", 60)
        ...
        elif conf < conf_floor:                       signal="WATCH"
        elif direction=="bullish":                    signal="BUY"
        elif direction=="bearish":                    signal="SELL"
        else:                                         signal="WATCH"

        profile=("STRONG" if conf>=75 and sg_passed and op_passed else
                 "MODERATE" if conf>=conf_floor else
                 "WEAK" if conf>=45 else "AVOID")
...
class ForwardOutcomeEngine:
    def measure(self,bar_signals,df,horizon,fwd_window):
        ...
        for bs in bar_signals:
            if bs.signal not in ("BUY","SELL"): continue
            ...
            outcomes.append(ForwardOutcome(
                ...
                entry=entry,target_1=t1,target_2=t2,stop_loss=sl,rrRatio=bs.rrRatio or 0,
...
class HorizonAgreementEngine:
    PRIO={"BUY":5,"SELL":5,"BLOCKED":3,"WATCH":2,"HOLD":1,"AVOID":0}
    ...
    def compute(self,sigs_by_hz:Dict[str,List[BarSignal]])->List[HorizonAgreement]:
        ...
        for date in sorted(by_date):
            ...
            buys=sum(1 for s in av if s == "BUY")
    ...
    for bar_idx in indices:
        ...
        if verbose: # Print all bars for debugging
            print(f"      {bar_date} | {bs.signal:<8} | conf={bs.confidence:.0f}% "
                  f"| setup={bs.setup_type:<24} | rr={bs.rrRatio or 0:.2f} | timeline={bs.est_time_str}"
...
def _stats(r:ReplayResult):
    ...
    for bs in r.bar_signals:
        dist[bs.signal]+=1
        if bs.signal == "BUY": bc.append(bs.confidence)
...
def write_signal_log(results):
    ...
    with open(p,"w",newline="", encoding="utf-8") as f:
        ...
        w.writerow(["symbol","horizon","date","bar_idx","signal","profile_signal",
                    ...
                    "rrRatio","sl_atr_mult","can_execute","is_hard_blocked",
        ...
        for r in results:
            for bs in r.bar_signals:
                w.writerow([bs.symbol,bs.horizon,bs.bar_date,bs.bar_idx,
                            ...
                            _fmt(bs.rrRatio),_fmt(bs.sl_atr_mult),
...
def write_summary_report(results,ags_by_sym,elapsed):
    ...
    for r in sorted(results,key=lambda x:(x.symbol,x.horizon)):
        ...
        A(f"  {r.symbol:<16} {r.horizon:<12} "
          f"{d.get('BUY',0):>5} {d.get('SELL',0):>5} "
    ...
    A("\n"+"═"*72)
    text="\n".join(L); p.write_text(text, encoding="utf-8"); return p
...
def main()->int:
    ...
    if args.report:
        ...
        print(rp.read_text(encoding="utf-8") if rp.exists() else "No report — run without --report first.")
        return 0
    ...
    for sym in symbols:
        ...
        for hz in horizons:
            ...
            buys=sum(r.signal_dist.get(s,0) for s in ("BUY","SELL"))
    ...
    if all_results:
        ...
        print(); print(rp.read_text(encoding="utf-8"))
    else:
